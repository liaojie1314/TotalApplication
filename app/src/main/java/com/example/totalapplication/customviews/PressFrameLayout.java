...
public class PressFrameLayout extends FrameLayout {
...

    {
        ...
        padding = dip2px(getContext(), 20);
        cornerRadius = dip2px(getContext(), 5);
        shadeOffset = dip2px(getContext(), 5);

        // View 的 background 为颜色或者图片的两种情况
        Drawable background = getBackground();
        if (background instanceof ColorDrawable) {
            colorBg = ((ColorDrawable) background).getColor();
            paintBg.setColor(colorBg);
        } else {
            if (((BitmapDrawable) background) != null) {
