...
public class HomeAdapter extends BaseQuickAdapter<ImomoeSearch, BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, ImomoeSearch item) {
        ...
        Glide.with(mContext).load(item.img).transform(new CenterCrop(),
                new GlideRoundTransform(helper.itemView.getContext(), 3))
                .diskCacheStrategy(DiskCacheStrategy.ALL)
                .into((ImageView) helper.getView(R.id.bangumi_item_image));
